...
Процедура  ДолжностиСотрудника()
	...
	КонецПроцедуры
&НаСервере	
Процедура ПрочитатьСостояниеРаботникаСклада()
	
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СостоянияРаботниковСкладаСрезПоследних.Состояние КАК Состояние,
		|	СостоянияРаботниковСкладаСрезПоследних.Организация КАК Организация
		|ИЗ
		|	РегистрСведений.СостоянияРаботниковСклада.СрезПоследних КАК СостоянияРаботниковСкладаСрезПоследних
		|ГДЕ
		|	СостоянияРаботниковСкладаСрезПоследних.РаботникСклада = &РаботникСклада";
	...
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	СостоянияРаботникаСклада.Очистить();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НоваяСтрока=СостоянияРаботникаСклада.Добавить();
		НоваяСтрока.Организация=ВыборкаДетальныеЗаписи.Организация;
		НоваяСтрока.Состояние=ВыборкаДетальныеЗаписи.Состояние;
	КонецЦикла;
