...
// Эмулирует в тонком клиенте системную процедуру ВыполнитьОбработкуЗаданий(), но может применяться и
// в толстом клиенте.
//
...
// Получает настройки выполнения регламентных заданий для файлового режима.
// 
// Возвращаемое значение:
//  Настройки - Структура.
//
...
// Получает РегламентноеЗадание из информационной
...
Функция ПолучитьРасписаниеРегламентногоЗаданияВСтруктуре(Знач Идентификатор) Экспорт
	...
	Возврат итWMSОбщегоНазначенияКлиентСервер.РасписаниеВСтруктуру(ПолучитьРегламентноеЗадание(Идентификатор).Расписание);
...
Процедура УстановитьРасписаниеРегламентногоЗаданияИзСтруктуры(Знач Идентификатор, Знач РасписаниеВСтруктуре) Экспорт
	...
	Задание.Расписание = итWMSОбщегоНазначенияКлиентСервер.СтруктураВРасписание(РасписаниеВСтруктуре);
	Задание.Записать();
...
// "Эмулирует" функцию ФоновыеЗадания.ПолучитьФоновыеЗадания() для серверной и файловой ИБ.
...
// Предназначена только для режима "файл-сервер", используется в процедуре
...
Процедура ОбновитьТаблицуРегламентныхЗаданий(Параметры) Экспорт 
    ТаблицаРегламентныеЗадания=ОбновитьТаблицуРегламентныхЗаданийФункция(Параметры);
	ПоместитьВоВременноеХранилище(ТаблицаРегламентныеЗадания,Параметры.АдресТаблицы);


КонецПроцедуры

Функция  ОбновитьТаблицуРегламентныхЗаданийФункция(Знач Параметры)
	
	Перем Задание, Идентификатор, ИдентификаторРегламентногоЗадания, Индекс, Обновляемое, Строки, Таблица, ТаблицаРегламентныеЗадания, ТекущиеЗадания;
	
	ИдентификаторРегламентногоЗадания=Параметры.ИдентификаторРегламентногоЗадания;
	ТаблицаРегламентныеЗадания=Параметры.ТаблицаРегламентныеЗадания;
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
		Возврат ТаблицаРегламентныеЗадания;

КонецФункции

Процедура ОбновитьСтрокуТаблицыРегламентныхЗаданий(Строка, Задание);
	ТекстНеОпределено="<не определено>";
...
Процедура ОбновитьТаблицуФоновыхЗаданий(Параметры) Экспорт 
	ОтборПоСостояниюАктивно=Параметры.ОтборПоСостояниюАктивно;
	ОтборПоСостояниюЗавершено= Параметры.ОтборПоСостояниюЗавершено;
	ОтборПоСостояниюЗавершеноАварийно= Параметры.ОтборПоСостояниюЗавершеноАварийно;
	ОтборПоСостояниюОтменено=Параметры.ОтборПоСостояниюОтменено;
	ОтбиратьПоРегламентномуЗаданию=Параметры.ОтбиратьПоРегламентномуЗаданию;
	РегламентноеЗаданиеДляОтбораИдентификатор=Параметры.РегламентноеЗаданиеДляОтбораИдентификатор;
	ПустойИдентификатор=Параметры.ПустойИдентификатор;
	ВидОтбораПоПериоду=Параметры.ВидОтбораПоПериоду;
	ОтборПериодС=Параметры.ОтборПериодС;
	ОтборПериодПо=Параметры.ОтборПериодПо;
	ТаблицаРегламентныеЗадания=Параметры.ТаблицаРегламентныеЗадания;
    ТекстНеОпределено="<не определено>";
    ТаблицаРегламентныеЗадания=ОбновитьТаблицуРегламентныхЗаданийФункция(новый Структура("ИдентификаторРегламентногоЗадания,ТаблицаРегламентныеЗадания",Неопределено,ТаблицаРегламентныеЗадания));

	
	
	ТаблицаФоновыеЗадания=Параметры.ТаблицаФоновыеЗадания;
	// 1. Подготовка отбора
	Отбор = Новый Структура;
	...
	ПоместитьВоВременноеХранилище(ТаблицаФоновыеЗадания,Параметры.АдресТаблицы);
	//Элементы.ТаблицаФоновыеЗадания.Обновить();
	
КонецПроцедуры


Процедура СообщенияПользователюИОписаниеИнформацииОбОшибкеВФоне(Параметры)Экспорт 
    ...
    ПоместитьВоВременноеХранилище(СообщенияПользователюИОписаниеИнформацииОбОшибке,Параметры.АдресСообщения);
	КонецПроцедуры