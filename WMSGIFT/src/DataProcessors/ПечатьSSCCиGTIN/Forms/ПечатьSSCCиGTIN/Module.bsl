...
Процедура ЗапроситьSSCCНаСервере()
	Данные=итWMSПривилегированныйМодуль.ПолучитьНастройкиИзХранилищаПоСвойствам("СоответствиеФилиаловИорганизация");
	Строка=Данные.СоответствиеФилиаловИорганизация.Найти(Организация);
	Если Строка=Неопределено Тогда
		Сообщить("Нет данных по организации");
		 Возврат;
	КонецЕсли;
	Филиал=Строка.Филиал;

	Если КоличествоSSCC=1 тогда
		SSCC=итWMSОбработчикSSCC.ПолучитьНовыйSSCC(Филиал,,ПараметрыСеанса.ТекущийПользователь.ФизическоеЛицо);
		НоваяСтрока=ТаблицаПолученныхSSCC_GTIN.Добавить();
		НоваяСтрока.Идентификатор=SSCC;
	ИначеЕсли КоличествоSSCC>1 тогда
		SSCCСписок=итWMSОбработчикSSCC.ПолучитьНовыеSSCC(Филиал,,ПараметрыСеанса.ТекущийПользователь.ФизическоеЛицо,КоличествоSSCC);
...
Процедура ЗапроситьGTINНаСервере()
	...
	Если КоличествоGTIN=1 тогда
		GTIN=итWMSОбработчикSSCC.ПолучитьНовыйSSCC(Филиал,,ПараметрыСеанса.ТекущийПользователь.ФизическоеЛицо,Перечисления.итWMSТипыЛогистическойЕденицыWMS.СборныйКороб);
		НоваяСтрока=ТаблицаПолученныхSSCC_GTIN.Добавить();
		НоваяСтрока.Идентификатор=GTIN;
	ИначеЕсли КоличествоGTIN>1 тогда
		GTINписок=итWMSОбработчикSSCC.ПолучитьНовыеSSCC(Филиал,,ПараметрыСеанса.ТекущийПользователь.ФизическоеЛицо,КоличествоGTIN,Перечисления.итWMSТипыЛогистическойЕденицыWMS.СборныйКороб);
...
Процедура НоменклатураПриИзмененииНаСервере()
	Если КрепостьПоНоменклатуре(Номенклатура)>0 Тогда 
...
КонецПроцедуры
&НаСервере
Функция КрепостьПоНоменклатуре(Номенклатура)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	алкСоответствияАлкогольнойПродукцииЕГАИСИНоменклатуры.АлкогольнаяПродукция.Крепость КАК Крепость
		|ИЗ
		...
		|ГДЕ
		|	алкСоответствияАлкогольнойПродукцииЕГАИСИНоменклатуры.Номенклатура = &Номенклатура";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат   ВыборкаДетальныеЗаписи.Крепость;
	КонецЦикла;
	    Возврат 0;
...
ДатаПроизводства=СерияНоменклатуры.ДатаПроизводства;
...
Процедура ОрганизацияКонтрагентПриИзмененииНаСервере()
	...
	КОДЕГАИСорг=ОрганизацияКонтрагент.FSRAR_ID;	
	КонецЕсли;
