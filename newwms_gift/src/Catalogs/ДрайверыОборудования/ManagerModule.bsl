...
Процедура ПриНачальномЗаполненииЭлементов(ДрайверыОборудования) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение предопределенных элементов драйвера'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
//
// Параметры:
//  ПараметрыДрайвера - Структура - см. МенеджерОборудованияКлиентСервер.ПараметрыСозданияНовогоДрайвера()
//
Процедура ЗаполнитьПредопределенныйЭлемент(ПараметрыДрайвера) Экспорт
	
	Попытка
		Драйвер = ПредопределенныйЭлемент("Справочник.ДрайверыОборудования." + ПараметрыДрайвера.ИмяДрайвера);
	Исключение
		Сообщение = НСтр("ru = 'Предопределенный элемент ""%Параметр%"" не найден.'");
		Сообщение = СтрЗаменить(Сообщение, "%Параметр%", "Справочник.ДрайверыОборудования." + ПараметрыДрайвера.ИмяДрайвера);
	...
	Если Драйвер = Неопределено Тогда  
		ПараметрыДрайвера.СпособПодключения = Перечисления.СпособПодключенияДрайвера.ИзМакета;
		ПараметрыДрайвера.Предопределенный  = Истина;
		СоздатьНовыйЭлемент(ПараметрыДрайвера);
	Иначе 
		Драйвер = Драйвер.ПолучитьОбъект();
		Драйвер.Заблокировать();
		Если (Драйвер.Наименование <> ПараметрыДрайвера.Наименование)
			Или (Драйвер.ИдентификаторОбъекта <> Строка(ПараметрыДрайвера.ИдентификаторОбъекта))
			Или (Драйвер.ВерсияДрайвера <> Строка(ПараметрыДрайвера.ВерсияДрайвера))
			Или (Драйвер.ИмяМакетаДрайвера <> Строка(ПараметрыДрайвера.ИмяМакетаДрайвера))
...
Функция ДрайвераПоТипуОборудования(ТипОборудования, ТолькоДоступные = Истина, СнятыеСПоддержкиДрайвера = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	%УСЛОВИЕ1%
		|	%УСЛОВИЕ2%
		|УПОРЯДОЧИТЬ ПО ДрайверыОборудованияПереопределяемый.Наименование";
		
	Если ТолькоДоступные Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%УСЛОВИЕ1%", "И НЕ ДрайверыОборудованияПереопределяемый.ПометкаУдаления");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%УСЛОВИЕ1%", "");
	КонецЕсли;
	
	Если НЕ СнятыеСПоддержкиДрайвера Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%УСЛОВИЕ2%", "И НЕ ДрайверыОборудованияПереопределяемый.СнятСПоддержки");
	Иначе
