...
Процедура ЗаполнитьПоДаннымМакета(ЧтениеИзФайла = Ложь)
	...
	Если ЧтениеИзФайла Тогда
		...
		Попытка
			ТекстовыйДокумент.Прочитать(ИмяФайла); // АПК: 566
	...
	Если ОборудованиеПринтерыЭтикетокВызовСервера.ПолучитьОписаниеМакета(Источник, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
...
Функция ПолучитьXMLОписаниеЭтикетки()
	...
	Возврат ОборудованиеПринтерыЭтикетокВызовСервера.СохранитьXMLФайл(ШиринаЭтикетки, ВысотаЭтикетки, Ориентация, Поля);
...
Функция СохранитьXMLФайл(СохранениеВФайл = Ложь)
	...
	Если СохранениеВФайл Тогда
		...
		Попытка
			ТекстовыйДокумент.Записать(ИмяФайла); // АПК: 566
			Возврат Содержимое;
		Исключение
