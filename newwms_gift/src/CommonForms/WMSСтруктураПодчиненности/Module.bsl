...
Процедура ВывестиТекущийДокумент(Макет)
	...
	Если Выборка.Следующий() Тогда
		...
		Если ПереопределяемоеПредставление <> Неопределено Тогда
			СтруктураРеквизитов = итWMSСлужебныеПроцедурыИФункции.СтрокаТаблицыЗначенийВСтруктуру(Выборка.Владелец().Выгрузить()[0]);
...
Функция ПолучитьВыборкуПоРеквизитамДокумента(ДокументСсылка)
	...
	|	Ссылка = &Ссылка
	|";
	ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеДокумента, "#Проведен", ИмяРеквизитаДокумента(МетаданныеДокумента.Имя, "Проведен"), "Проведен");
	...
	ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеДокумента, "#Валюта", ИмяРеквизитаДокумента(МетаданныеДокумента.Имя, "ВалютаДокумента"), "ВалютаДокумента");
...
Процедура ВывестиРодительскиеДокументы(ТекущийДокумент,ДеревоРодитель)
	...
	Для Каждого Реквизит ИЗ МетаданныеДокумента.Реквизиты Цикл
		...
		Если Не Метаданные.КритерииОтбора.WMSСвязанныеДокументы.Состав.Содержит(Реквизит) Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого ТекущийТип Из Реквизит.Тип.Типы() Цикл
			...
			Если МетаданныеРеквизита <> Неопределено
			   И РеквизитПодходящегоТипа(МетаданныеРеквизита)
			   И ПравоДоступа("Чтение", МетаданныеРеквизита) Тогда

			    ЕстьРеквизитДаты = Метаданные.Документы.Содержит(МетаданныеРеквизита);
				...
				Если ЗначениеЗаполнено(ЗначениеРеквизита)
					...
					Если ЕстьРеквизитДаты Тогда 
						СписокРеквизитов.Добавить(ЗначениеРеквизита,Формат(ЗначениеРеквизита.Дата,"ДФ=yyyyMMddЧЧММсс"));
					Иначе
						СписокРеквизитов.Добавить(ЗначениеРеквизита,Формат(Дата(1,1,1),"ДФ=yyyyMMddЧЧММсс"));
					КонецЕсли;
		...
		Для Каждого КолонкаТЧ ИЗ СодержимоеТЧ.Колонки Цикл
			...
			Для Каждого СтрокаТЧ ИЗ СодержимоеТЧ Цикл
				...
				Если МетаданныеЗначения <> Неопределено Тогда
					...
					ЕстьРеквизитДаты = Метаданные.Документы.Содержит(МетаданныеЗначения);
					
					Если ЗначениеЗаполнено(ЗначениеРеквизита)
						И РеквизитПодходящегоТипа(МетаданныеЗначения)
						И ЗначениеРеквизита <> ТекущийДокумент
						И СписокРеквизитов.НайтиПоЗначению(ЗначениеРеквизита) = Неопределено Тогда
...
Процедура ЗаменитьТекстЗапроса(ТекстЗапроса, МетаданныеДокумента, ЧтоЗаменять, ИмяРеквизита, Представление)
	...
	ИначеЕсли Метаданные.Документы.Содержит(МетаданныеДокумента) И ИмяРеквизита = "Проведен" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ЧтоЗаменять, ИмяРеквизита + " КАК " + Представление);
	ИначеЕсли ИмяРеквизита = "Проведен" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ЧтоЗаменять, " ЛОЖЬ КАК " + Представление);
	Иначе
...
Процедура ДополнитьКэшМетаданных(МетаданныеДокумента, ИмяДокумента,КэшРеквизитовДокумента)
	...
	Если РеквизитыДокумента = Неопределено Тогда
		...
		ИмяРеквизитаВалюта = ИмяРеквизитаДокумента(МетаданныеДокумента.Имя, "ВалютаДокумента");
		Если МетаданныеДокумента.Реквизиты.Найти(ИмяРеквизитаВалюта) <> Неопределено Тогда
			РеквизитыДокумента.Вставить("ВалютаДокумента", ИмяРеквизитаВалюта);
		Иначе
			РеквизитыДокумента.Вставить("ВалютаДокумента", "NULL");
		КонецЕсли;
		
		ИмяРеквизитаДата = ИмяРеквизитаДокумента(МетаданныеДокумента.Имя, "Дата");
		Если МетаданныеДокумента.Реквизиты.Найти(ИмяРеквизитаДата) <> Неопределено Тогда
			РеквизитыДокумента.Вставить("Дата", ИмяРеквизитаДата);
		ИначеЕсли Метаданные.Документы.Содержит(МетаданныеДокумента) Тогда
			РеквизитыДокумента.Вставить("Дата", "Дата");
		Иначе
			РеквизитыДокумента.Вставить("Дата", "ДАТАВРЕМЯ(1,1,1)");
		КонецЕсли;
		
		ИмяРеквизитаПроведен = ИмяРеквизитаДокумента(МетаданныеДокумента.Имя, "Проведен");
		Если МетаданныеДокумента.Реквизиты.Найти(ИмяРеквизитаПроведен) <> Неопределено Тогда
			РеквизитыДокумента.Вставить("Проведен", ИмяРеквизитаПроведен);
		ИначеЕсли Метаданные.Документы.Содержит(МетаданныеДокумента) Тогда
...
Функция ПолучитьСписокДокументовПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	Если Метаданные.КритерииОтбора.WMSСвязанныеДокументы.Тип.СодержитТип(ТипЗнч(ЗначениеКритерияОтбора))  Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СвязанныеДокументы.Ссылка
		|ИЗ
		|	КритерийОтбора.WMSСвязанныеДокументы(&ЗначениеКритерияОтбора) КАК СвязанныеДокументы";
...
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент,ДеревоРодитель)
	...
	Для Каждого КлючИЗначение Из КэшПоТипамДокументов Цикл
		...
		|" + КэшРеквизитовДокумента[КлючИЗначение.Ключ]["Дата"] + " КАК Дата,
		|	Ссылка,
		|" + КэшРеквизитовДокумента[КлючИЗначение.Ключ]["Проведен"] + " КАК Проведен,
		|	ПометкаУдаления,
		|" + КэшРеквизитовДокумента[КлючИЗначение.Ключ]["СуммаДокумента"] + " КАК СуммаДокумента,
		|" + КэшРеквизитовДокумента[КлючИЗначение.Ключ]["ВалютаДокумента"] + " КАК ВалютаДокумента,
		|	#Представление
		|ИЗ
		|	" + КлючИЗначение.Ключ + "
		|ГДЕ
		|	Ссылка В (&" + СтрЗаменить(КлючИЗначение.Ключ, ".", "_") + ")";
...
Функция ИмяРеквизитаДокумента(ИмяДокумента, Реквизит) 
	...
	ИначеЕсли Реквизит = "ВалютаДокумента" Тогда
		Возврат ?(ИмяРеквизитаДокумента = Неопределено,"ВалютаДокумента",ИмяРеквизитаДокумента);
	КонецЕсли;
	
	Возврат ?(ИмяРеквизитаДокумента = Неопределено, Реквизит,ИмяРеквизитаДокумента);
	
КонецФункции

Функция РеквизитПодходящегоТипа(МетаданныеРеквизита)
	Результат = Метаданные.Документы.Содержит(МетаданныеРеквизита); //ИЛИ 
//	МетаданныеРеквизита = Метаданные.Справочники.ГосударственныеКонтракты ИЛИ
//	МетаданныеРеквизита = Метаданные.Справочники.КонтрактыСЗаказчиками ИЛИ
//	МетаданныеРеквизита = Метаданные.Справочники.КонтрактыИсполнителей;
	Возврат Результат;
КонецФункции
...
Процедура ПометитьНаУдаление(Команда)
	ТекущийДокумент = Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка;	
    ПометитьНаУдалениеНаСервере(ТекущийДокумент);	
    ВывестиДляТекущего("");
КонецПроцедуры

Процедура ПометитьНаУдалениеНаСервере(ТекущийДокумент)
	ОбъектДокумента=ТекущийДокумент.ПолучитьОбъект();
	ОбъектДокумента.Записать(РежимЗаписиДокумента.ОтменаПроведения);
	Если  не ОбъектДокумента.ПометкаУдаления Тогда 
...
Процедура Проведение(Команда)
	ТекущийДокумент = Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка;	
    ПроведениеНаСервере(ТекущийДокумент);	
    ВывестиДляТекущего("");
КонецПроцедуры

Процедура ПроведениеНаСервере(ТекущийДокумент)
ОбъектДокумента=ТекущийДокумент.ПолучитьОбъект();
ОбъектДокумента.Записать(РежимЗаписиДокумента.Проведение);
	КонецПроцедуры

&НаСервере
Процедура ОтменаПроведенияНаСервере(ТекущийДокумент)
ОбъектДокумента=ТекущийДокумент.ПолучитьОбъект();
ОбъектДокумента.Записать(РежимЗаписиДокумента.ОтменаПроведения);
КонецПроцедуры
...
ТекущийДокумент = Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка;	
ОтменаПроведенияНаСервере(ТекущийДокумент);
ВывестиДляТекущего("");
КонецПроцедуры
