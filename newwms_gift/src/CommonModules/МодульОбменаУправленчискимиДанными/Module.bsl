...
Функция СоздатьЭлементКлассификаторСправокАиБКА11(СтрокаДанных,УзелОбмена,СтруктураДанных)
    ...
    Если СтрокаДанных.Родитель<>ПустойИдентификатор() Тогда
    	Родитель=ПоискЭлементаСоответствияОбмена(СтрокаДанных.Родитель,УзелОбмена);
    	Если Родитель=Неопределено Тогда
    		СтрокиСРодителем=СтруктураДанных.КлассификаторСправокАиБ.НайтиСтроки(новый Структура("Ссылка",СтрокаДанных.Родитель ));
    		Если СтрокиСРодителем.Количество()>0 Тогда
    ...
    Если Организация=Неопределено Тогда
    	СтруктураДанных.Вставить("Статус",404);
	    СтруктураДанных.Вставить("ОписаниеОшибки","Объект Классификатор не был изменен,т.к не найдена организация "+СтрокаДанных.код);
    	Возврат СсылкаКлассификатора;
    КонецЕсли;
...
НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
    ...
    Если Владелец=Неопределено Тогда
    	СтруктураДанных.Вставить("Статус",404);
	    СтруктураДанных.Вставить("ОписаниеОшибки","Объект Классификатор не был изменен,т.к не найден владелец "+СтрокаДанных.код);
    	Возврат;
	КонецЕсли;
	Организация=Неопределено;
    Если СтрокаДанных.Организация<>ПустойИдентификатор() Тогда
...
Функция СоздатьОбъектКонтрАгентаКА11(СтрокаДанных,УзелОбмена,СтруктураДанных)
	...
	ЗаполнитьЗначенияСвойств(НовыйОбъект,СтрокаДанных,,"Ссылка,Родитель,ИНН,КПП,ит_WMS_ВидСкладскойДеятельности,НаименованиеПолное,итОтгрузкаПолнымиПалетами,итПроцентДопустимогоОстаточногоСрокаГодности");
    Иначе
...
НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
	...
	ОбъектИзменения=СсылкаНаОбъект.ПолучитьОбъект();
	Если СтрокаДанных.ЭтоГруппа Тогда 
	ЗаполнитьЗначенияСвойств(ОбъектИзменения,СтрокаДанных,,"Ссылка,Родитель,ИНН,КПП,ит_WMS_ВидСкладскойДеятельности,НаименованиеПолное,итОтгрузкаПолнымиПалетами,итПроцентДопустимогоОстаточногоСрокаГодности");
	Иначе 
	ЗаполнитьЗначенияСвойств(ОбъектИзменения,СтрокаДанных,,"Ссылка,Родитель");
	КонецЕсли;
	ОбъектИзменения.ОбменДанными.Отправитель=УзелОбмена;
...
Функция СоздатьОбъектДоговораКонтрагентаКА11(СтрокаДанных,УзелОбмена,СтруктураДанных)
    ...
    Если СтрокаДанных.Родитель<>ПустойИдентификатор() Тогда
    	Родитель=ПоискЭлементаСоответствияОбмена(СтрокаДанных.Родитель,УзелОбмена);
    	Если Родитель=Неопределено Тогда
    		СтрокиСРодителем=СтруктураДанных.ДоговорыКонтрагентов.НайтиСтроки(новый Структура("Ссылка",СтрокаДанных.Родитель ));
    		Если СтрокиСРодителем.Количество()>0 Тогда
...
Функция СоздатьОбъектНоменклатурыКА11(СтрокаДанных,УзелОбмена,СтруктураДанных)
	...
	ЗаполнитьЗначенияСвойств(НовыйОбъект,СтрокаДанных,,"Ссылка,Родитель,ЕдиницаХраненияОстатков,ЕдиницаИзмеренияМест,итИдентификаторНоменклатурнойПозиции,СрокГодности,Производитель");
    Иначе
    НовыйОбъект=Справочники.Номенклатура.СоздатьЭлемент();	
	ЗаполнитьЗначенияСвойств(НовыйОбъект,СтрокаДанных,,"Ссылка,Родитель,ЕдиницаХраненияОстатков,ЕдиницаИзмеренияМест,итИдентификаторНоменклатурнойПозиции,Производитель");
	...
	Если ЕдиницаХраненияОстатков=Неопределено Тогда
		СтрокиСЕД=СтруктураДанных.ЕдИзмерения.НайтиСтроки(новый Структура("Ссылка",СтрокаДанных.ЕдиницаХраненияОстатков ));
...
Процедура ИзменитьДанныеНоменклатурыКА11(СтрокаДанных,УзелОбмена,СтруктураДанных,СсылкаНаОбъект)
	...
	Если ОбъектИзменения.ЭтоГруппа Тогда 
		ЗаполнитьЗначенияСвойств(ОбъектИзменения,СтрокаДанных,,"Ссылка,Родитель,ЕдиницаХраненияОстатков,ЕдиницаИзмеренияМест,итИдентификаторНоменклатурнойПозиции,СрокГодности,Производитель");
	иначе
		ЗаполнитьЗначенияСвойств(ОбъектИзменения,СтрокаДанных,,"Ссылка,Родитель,ЕдиницаХраненияОстатков,ЕдиницаИзмеренияМест,итИдентификаторНоменклатурнойПозиции,Производитель");
...
Процедура ПолучитьДанныеЗаказаПокупателяКА11(СтруктураДанных,УзелОбмена,Отказ)
	...
	Для Каждого стр из ЗаказыПокупателей Цикл 
		...
		Если Контрагент=Неопределено Тогда 
			СтруктураДанных.Вставить("Статус",404);
			СтруктураДанных.Вставить("ОписаниеОшибки","Объект Контрагент не был найден "+Строка(стр.Контрагент)+" для заказа "+стр.НомерИсточника);
			Отказ=Истина;
			ОтменитьТранзакцию();
			Возврат;
        КонецЕсли;
		...
		Если Склад=Неопределено Тогда 
			СтруктураДанных.Вставить("Статус",404);
			СтруктураДанных.Вставить("ОписаниеОшибки","Объект Склад не был найден "+Строка(стр.Склад)+" для заказа "+стр.НомерИсточника);
			Отказ=Истина;
			ОтменитьТранзакцию();
			Возврат;
		КонецЕсли;
		...
		Для Каждого СтрТч из стр.ЗаказТч Цикл 
			...
			Если СтрТч.Номенклатура<>ПустойИдентификатор() Тогда 
				...
				Если Номенклатура=Неопределено Тогда 
					СтруктураДанных.Вставить("Статус",404);
					СтруктураДанных.Вставить("ОписаниеОшибки","Объект Номенклатура не был найден "+Строка(стр.Номенклатура)+" для заказа "+стр.НомерИсточника);
					Отказ=Истина;
					ОтменитьТранзакцию();
					Возврат;
				КонецЕсли;
			...
			Если СтрТч.ЕдиницаИзмеренияМест<>ПустойИдентификатор() Тогда 
				...
				Если ЕдиницаИзмеренияМест=Неопределено Тогда 
					СтруктураДанных.Вставить("Статус",404);
					СтруктураДанных.Вставить("ОписаниеОшибки","Объект ЕдиницаИзмеренияМест не был найден "+Строка(СтрТч.ЕдиницаИзмеренияМест)+" для заказа "+стр.НомерИсточника);
					Отказ=Истина;
					ОтменитьТранзакцию();
					Возврат;
				КонецЕсли;
			...
			Если СтрТч.ЕдиницаИзмерения<>ПустойИдентификатор() Тогда 
				...
				Если ЕдиницаИзмерения=Неопределено Тогда 
					СтруктураДанных.Вставить("Статус",404);
					СтруктураДанных.Вставить("ОписаниеОшибки","Объект ЕдиницаИзмерения не был найден "+Строка(СтрТч.ЕдиницаИзмерения)+" для заказа "+стр.НомерИсточника);
					Отказ=Истина;
					ОтменитьТранзакцию();
					Возврат;
				КонецЕсли;
	...
	Для Каждого стр из ЗаказыПоставщиков Цикл 
		...
		Для Каждого СтрТч из стр.ТоварыТЧ Цикл 
			...
			иначе
				...
				Если Номенклатура=Неопределено Тогда 
					СтруктураДанных.Вставить("Статус",404);
					СтруктураДанных.Вставить("ОписаниеОшибки","Объект Номенклатура не был найден "+Строка(СтрТч.Номенклатура)+" для заказа "+стр.НомерИсточника);
					Отказ=Истина;
					ОтменитьТранзакцию();
					Возврат;
				КонецЕсли;
			...
			иначе	
				...
				Если ХарактеристикаНоменклатуры=Неопределено Тогда 
					СтруктураДанных.Вставить("Статус",404);
					СтруктураДанных.Вставить("ОписаниеОшибки","Объект ХарактеристикаНоменклатуры не был найден "+Строка(СтрТч.ХарактеристикаНоменклатуры)+" для заказа "+стр.НомерИсточника);
					Отказ=Истина;
					ОтменитьТранзакцию();
					Возврат;
				КонецЕсли;
...
Процедура ПолучениеДанныхСоотвАлкПродукцииИНмКА11(СтруктураДанных,УзелОбмена,Отказ)
	...
	Для Каждого стр из СоотвНмиАлкПродукции   Цикл 
		...
		Если АлкогольнаяПродукция=Неопределено Тогда 
			СтруктураДанных.Вставить("Статус",404);
			СтруктураДанных.Вставить("ОписаниеОшибки","Объект АлкогольнаяПродукция не был найден "+Строка(стр.АлкогольнаяПродукция));
			Отказ=Истина;
			ОтменитьТранзакцию();
			Возврат;
		КонецЕсли;
...
Исключение
	...
	ЗаписьЖурналаРегистрации("ОбменКА11",УровеньЖурналаРегистрации.Ошибка,Метаданные.ПланыОбмена.ОбменКА11,ТекстОшибки);
КонецПопытки;
Если ТипЗнч(Ответ)<>Тип("Структура") Тогда 
	Сообщить("ошибка формата данных");
	ЗаписьЖурналаРегистрации("ОбменКА11",УровеньЖурналаРегистрации.Ошибка,Метаданные.ПланыОбмена.ОбменКА11,"ошибка формата данных");
КонецЕсли;
Если Ответ.Свойство("Статус") Тогда 
	Если Ответ.Статус=404 тогда
		Сообщить(Ответ.ОписаниеОшибки);
		ЗаписьЖурналаРегистрации("ОбменКА11",УровеньЖурналаРегистрации.Ошибка,Метаданные.ПланыОбмена.ОбменКА11,Ответ.ОписаниеОшибки);
		Возврат;
	КонецЕсли;
КонецЕсли;
...
Процедура ЗаполнитьСерииНоменклатурыКА11(СтруктураОбменаДанными,Знач Узел)
	...
	НоваяСтрока.Производитель=ПоискЭлементаСоответствияОбменаДругойИБСТипомПриемника(ВыборкаДетальныеЗаписи.Производитель,Узел);
...
Процедура ЗаполнитьДаннымиПТУ(СтруктураОбменаДанными,Узел)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			НоваяСтрокаТовары.СерияНоменклатурыВДругойИБ=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.СерияНоменклатуры,Узел);
            НоваяСтрокаТовары.ХарактеристикаНоменклатуры=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ХарактеристикаНоменклатуры,Узел);
			...
			НоваяСтрокаТовары.ЕдиницаИзмерения=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ЕдиницаИзмерения,Узел);
...
Процедура ЗаполнитьДаннымиПеремещения(СтруктураОбменаДанными,Узел)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрока.Организация=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаДетальныеЗаписи.Организация,Узел);
		НоваяСтрока.СкладОтправитель=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаДетальныеЗаписи.СкладОтправитель,Узел);
		НоваяСтрока.СкладПолучатель=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаДетальныеЗаписи.СкладПолучатель,Узел);
		НоваяСтрока.Комментарий=ВыборкаДетальныеЗаписи.Комментарий;
		НоваяСтрока.Номер=ВыборкаДетальныеЗаписи.Номер;
		...
		Пока ВыборкаТовары.Следующий() Цикл
            ...
            НоваяСтрокаТовары.Качество=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.Качество,Узел);
			НоваяСтрокаТовары.ЕдиницаИзмерения=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ЕдиницаИзмерения,Узел);
			НоваяСтрокаТовары.ЕдиницаИзмеренияМест=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ЕдиницаИзмеренияМест,Узел);
		...
		Если ТипЗнч(ВыборкаДетальныеЗаписи.Основание)=Тип("ДокументСсылка.итWMSНаборка") Тогда 
			Если ТипЗнч(ВыборкаДетальныеЗаписи.Основание.итОснование)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
				Если не ВыборкаДетальныеЗаписи.Основание.итОснование.Пустая() Тогда 
					НоваяСтрока.ДокументРезерва=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаДетальныеЗаписи.Основание.итОснование,Узел);	
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
        ТаблицаТовары=новый ТаблицаЗначений;
...
Процедура ЗаполнитьДаннымиДокументаКорректировкиСерий(СтруктураОбменаДанными,Узел) 
	...
	Пока ВыборкаТовары.Следующий() Цикл   
		...
		НоваяСтрокаТовары.СерияНоменклатурыНоваяВДругойИБ=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.СерияНоменклатурыНовая,Узел);   
		НоваяСтрокаТовары.СерияНоменклатурыСтарая=ВыборкаТовары.СерияНоменклатурыСтарая.УникальныйИдентификатор();
		НоваяСтрокаТовары.СерияНоменклатурыСтараяВДругойИБ=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.СерияНоменклатурыСтарая,Узел);  
		...
		НоваяСтрокаТовары.СерияНоменклатурыСтараяИмяТипаПриемника=ВыборкаТовары.СерияНоменклатурыСтарая.Метаданные().ПолноеИмя();
       	НоваяСтрокаТовары.ХарактеристикаНоменклатурыНовая=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ХарактеристикаНоменклатурыНовая,Узел);
        НоваяСтрокаТовары.ХарактеристикаНоменклатурыСтарая=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ХарактеристикаНоменклатурыСтарая,Узел);
        НоваяСтрокаТовары.ЕдиницаИзмерения=ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаТовары.ЕдиницаИзмерения,Узел);
...
МассивРезультатов = Запрос.ВыполнитьПакет();
	...
	Пока ВыборкаСправкаБ.Следующий() Цикл
	 СтруктураМассива=новый Структура;
		СтруктураМассива.Вставить("СправкаБ",ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаСправкаБ.СправкаБ,УзелОбмена));
...
Функция  ЗаписьПодвержденногоДокументаЕГАИС(СтруктураДанных)
	 Реализация=Документы.РеализацияТоваровУслуг.ПолучитьСсылку(новый УникальныйИдентификатор(СтруктураДанных.СсылкаВДругойИБ));
...
Функция  ПолучитьДанныеМарокПоРеализацииКОА(СтруктураДанных)
	...
	Пока ВыборкаПоСправкеБ.Следующий() Цикл
		СтруктураМассива=новый Структура;
		СтруктураМассива.Вставить("СправкаБ",ПоискЭлементаСоответствияОбменаДругойИБ(ВыборкаПоСправкеБ.СправкаБ,УзелОбмена));
		ТаблицаМарок=новый ТаблицаЗначений;
		ТаблицаМарок.Колонки.Добавить("Марка");
		ТаблицаМарок.Колонки.Добавить("GTIN");
...
Функция ПоискЭлементаСоответствияОбменаДругойИБСТипомПриемника(СобственнаяСсылка,УзелОбмена)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтруктураВозврата.Вставить("СсылкаВДругойИБ",новый УникальныйИдентификатор(ВыборкаДетальныеЗаписи.СсылкаВДругойИБ));
