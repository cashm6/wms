...
//  Структура.
Функция ПараметрыОперацииТСД() Экспорт
			...
			Пока ЧтениеXML.Прочитать() Цикл  
				Если ЧтениеXML.Имя = "Record" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					Если Не ПустаяСтрока(ШтрихкодBase64) Тогда
						Штрихкод = МенеджерОборудованияВызовСервера.Base64ВШтрихкод(ШтрихкодBase64);
					КонецЕсли;
					Если Не ПустаяСтрока(ШтрихкодУпаковкиBase64) Тогда
						ШтрихкодУпаковки = МенеджерОборудованияВызовСервера.Base64ВШтрихкод(ШтрихкодУпаковкиBase64);
					КонецЕсли;
...
Функция СформироватьТаблицуТоваровТСД(ДанныеЗагрузки, ПолнаяВыгрузка, РевизияИнтерфейса = 0) 
	...
	Для Каждого Позиция Из ДанныеЗагрузки  Цикл
		...
		Если РевизияИнтерфейса >= 3003 Тогда
			...
			Иначе
				ШтрихкодBase64 = МенеджерОборудованияВызовСервера.ШтрихкодВBase64(Позиция.Штрихкод)
			КонецЕсли;
			...
			ИначеЕсли Позиция.Свойство("ШтрихкодУпаковки") И Не ПустаяСтрока(Позиция.ШтрихкодУпаковки) Тогда
				ШтрихкодУпаковкиBase64 = МенеджерОборудованияВызовСервера.ШтрихкодВBase64(Позиция.ШтрихкодУпаковки);
			КонецЕсли;
			Если Не ПустаяСтрока(ШтрихкодУпаковкиBase64) Тогда 
		...
		Если Алкоголь = Истина Тогда
			...
			Если РевизияИнтерфейса >= 3003 Тогда
				...
				Иначе
					ШтрихкодМаркиАлкогольнойПродукцииBase64 = МенеджерОборудованияВызовСервера.ШтрихкодВBase64(Позиция.ШтрихкодМаркиАлкогольнойПродукции)
				КонецЕсли;
