...
Процедура ВыполнитьЗагрузкуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка товаров из ККТ.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьВыгрузкуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Выгрузка товаров на ККТ.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьОчисткуТоваров(Настройки, Товары, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Очистка товаров на ККТ.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьЗагрузкуСпискаДокументов(Настройки, КассовыеДокументы, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка кассовых документов на ККТ.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьЗагрузкуМагазинов(Настройки, СписокМагазинов, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Истина) Экспорт
	...
	ЗаголовокЖурнала = НСтр("ru = 'Загрузка магазинов Эвотор.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция HTTPСоединение(Настройки)
	...
	СтруктураURL = ОбщегоНазначенияКлиентСервер.СтруктураURI("https://api.evotor.ru/");
...
Процедура ВыполнитьЗапросКСерверуЭвотор(Соединение, Настройки, Данные, Отказ, СообщениеОбОшибке)
	...
	Исключение
		...
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'НТТР запрос к облаку Эвотор.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
		Возврат;
	КонецПопытки;
...
Процедура ОбработатьОтветСервера(Результат, Настройки, Отказ, СообщениеОбОшибке)
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ответ сервера Эвотор.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, СообщениеОбОшибке);
	КонецЕсли;
...
Функция ПреобразоватьВСтрокуИзJSON(Строка, Отказ, СообщениеОбОшибке)
	...
	Исключение
		СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Чтение JSON.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция СтрокаJSON(Знач ПараметрыМетода, Отказ, СообщениеОбОшибке)
	...
	Исключение
		СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Отказ = Истина;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Запись JSON.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
