...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	ОбрабработатьМассивПараметров(МассивПараметров);
...
КонецПроцедуры
//++БЕН
Процедура ОбрабработатьМассивПараметров(МассивПараметров)
	Для Каждого стр из МассивПараметров Цикл 
		Если стр.ВычисляемоеЗначение Тогда 
			стр.ЗначениеВФорме=ОбработкаПроцедурыВыполнить(стр.КодОбработки);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Функция ОбработкаПроцедурыВыполнить(КодОбработки)
