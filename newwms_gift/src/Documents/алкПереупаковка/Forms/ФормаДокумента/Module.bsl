...
&НаСервере
Процедура ЗаполнитьSSCCиGTINКарантинаНаСервере()
	SSCCКарантина=Неопределено;
	GTINКарантина=Неопределено;
	СтруктураХраненияДанных=итWMSПривилегированныйМодуль.ПолучитьНастройкиИзХранилищаПоСвойствам("SSCCКарантина,GTINКарантина");
	СтруктураХраненияДанных.Свойство("SSCCКарантина",SSCCКарантина); 
	СтруктураХраненияДанных.Свойство("GTINКарантина",GTINКарантина); 
	Если SSCCКарантина=Неопределено или   GTINКарантина=Неопределено Тогда 
		Сообщить("Не установленны SSCC или GTIN карантина");
		Возврат
	КонецЕсли;	
	Для Каждого  стр из Объект.АкцизныеМарки Цикл 
		стр.Упаковка= GTINКарантина;
	КонецЦикла;
	Объект.Упаковки.Очистить();
	СтрокаУпаковки=Объект.Упаковки.Добавить();
	СтрокаУпаковки.Упаковка=GTINКарантина;
	СтрокаУпаковки.ИерархияУпаковки=SSCCКарантина;
КонецПроцедуры


&НаКлиенте
Процедура ЗаполнитьSSCCиGTINКарантина(Команда)
	ЗаполнитьSSCCиGTINКарантинаНаСервере();
КонецПроцедуры
