...
Процедура ПередЗаписьюПроведение(Отказ, РежимЗаписи, РежимПроведения)
	Если СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Создан и НеРаспределятьНаТСД или  НеВсеЯчейкиПикингаЗаполнены()  тогда
	...
	ИначеЕсли СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Создан и не НеРаспределятьНаТСД и не НеВсеЯчейкиПикингаЗаполнены()   тогда		
	...
	ИначеЕсли СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Зарезервирован и не НеРаспределятьНаТСД и не НеВсеЯчейкиПикингаЗаполнены()   тогда
		СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Распределяется;
		ПроверкаНаФиксациюЗадачДокументаДоРаспределения();
		для Каждого стр из Товары цикл
			Если не стр.ФиксацияСтроки Тогда 
...
Процедура ПроверкаНаСоответсвиеОснованиюЗаказПокупателя(Отказ)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Создан или
			СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Зарезервирован или
			СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Распределяется или
			...
			Если ВыборкаДетальныеЗаписи.КоличествоПланЗаказа <> ВыборкаДетальныеЗаписи.КоличествоВНаборке Тогда 
				итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю("Количетсво номенклатуры "+Строка(ВыборкаДетальныеЗаписи.Номенклатура) +" в заказе не равно количеству в наборке (в заказе "+Строка(ВыборкаДетальныеЗаписи.КоличествоПланЗаказа)+ 
		...
		иначе
			Если ВыборкаДетальныеЗаписи.КоличествоПланЗаказа <> ВыборкаДетальныеЗаписи.КоличествоФакт Тогда 
				итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю("Количетсво номенклатуры "+Строка(ВыборкаДетальныеЗаписи.Номенклатура) +" в заказе не равно  набранному количеству в наборке (в заказе "+Строка(ВыборкаДетальныеЗаписи.КоличествоПланЗаказа)+ 
				" в наборке " +Строка(ВыборкаДетальныеЗаписи.КоличествоФакт)+")");
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверкаНаДвижениеНеКратноУпаковки(Отказ, РежимЗаписи, РежимПроведения)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ВыборкаДетальныеЗаписи.НеКратноУпаковке Тогда 
			итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю("в индентификаторе "+ВыборкаДетальныеЗаписи.ИдентификаторУпаковки +" из ячейки " + ВыборкаДетальныеЗаписи.Ячейка +" забирается количество номенклатуры "+ВыборкаДетальныеЗаписи.Номенклатура+" не кратное упаковке ");
		КонецЕсли;
		Если ВыборкаДетальныеЗаписи.НеЗаданыДанныеЕдМест Тогда 
			итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю("в индентификаторе "+ВыборкаДетальныеЗаписи.ИдентификаторУпаковки +" из ячейки " + ВыборкаДетальныеЗаписи.Ячейка +" для номенклатуры "+ВыборкаДетальныеЗаписи.Номенклатура+" не заданы параметры ед. мест ");
		КонецЕсли;
...
Процедура ПередЗаписьюОтменаПроведения(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИначеЕсли СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Распределен тогда
		Отказ = Истина;
		итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю("Документ распределен на тсд и не может быть распроведен ");
...
ОписаниеОшибки="";
ОбработчикОсобенностейКонтрагентов.ЦентральныйОбработчикОсобенностей(ЭтотОбъект,Контрагент,,Дата,Организация,Отказ,ОписаниеОшибки);	
Если Отказ Тогда 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если  СтатусДокумента= Перечисления.итWMSСтатусыСкладскихДокументов.ВыполненСОшибкой Тогда 
		...
		ФоновыеЗадания.Выполнить("итWMSПривилегированныйМодуль.СоздатьЗадачиКИнвентаризацииОтмененныхЗадач",Массив,Строка(новый УникальныйИдентификатор),"СоздатьЗадачиКИнвентаризацииОтмененныхЗадач");
	КонецЕсли;
	ПеревестиЗаказВСостояниеНабора();
	КонецПроцедуры
...
Процедура УстановитьБлокировкиДанныхИОчиститьРегистры()
	...
	ТаблицаТоваровДляБлокировки.Колонки.Добавить("ЯчейкаПикинга",новый ОписаниеТипов("СправочникСсылка.итСкладскиеЯчейки"));
	ТаблицаТоваровДляБлокировки.Колонки.Добавить("Номенклатура",новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаТоваровДляБлокировки.Колонки.Добавить("ИдентификаторУпаковки",новый ОписаниеТипов("Строка",,,,новый КвалификаторыСтроки(172)));    
	
	ТаблицаТоваровДляБлокировкиХранения=новый ТаблицаЗначений;
	ТаблицаТоваровДляБлокировкиХранения.Колонки.Добавить("ЯчейкаОтправитель",новый ОписаниеТипов("СправочникСсылка.итСкладскиеЯчейки"));
	ТаблицаТоваровДляБлокировкиХранения.Колонки.Добавить("ИдентификаторУпаковки",новый ОписаниеТипов("Строка",,,,новый КвалификаторыСтроки(172)));  
	...
	СтруктураПараметров=итWMSСлужебныеПроцедурыИФункции.СоздатьСтруктуруПараметровБлокировкиДанных(ТаблицаТоваровДляБлокировки,БлокировкаДанных);
	...
	итWMSСлужебныеПроцедурыИФункции.УстановкаЭлементаБлокировокДанныхWMS(СтруктураПараметров); 
    Если РежимТестирования Тогда 	
	СтруктураПараметров=итWMSСлужебныеПроцедурыИФункции.СоздатьСтруктуруПараметровБлокировкиДанных(ТаблицаТоваровДляБлокировкиХранения,БлокировкаДанных);
...
Процедура РаспределениеДокументаНаЗадачиТСД(Отказ, РежимПроведения)
	...
	Пока ВыборкаИдентификаторУпаковкиПолучатель.Следующий() Цикл
		...
		КонецЕсли;
		Если ВыборкаИдентификаторУпаковкиПолучатель.ЯчейкаПикингаДляСборныйхПаллет<> Справочники.итСкладскиеЯчейки.ПустаяСсылка() тогда
		...
		ЗаполнитьСтрокиЗадачиТСД(Отказ, РежимПроведения,НоваяЗапись.ИдЗадачи,ВыборкаИдентификаторУпаковкиПолучатель,ТаблицаДляГруппировкиДанных);
		НоваяЗапись.ДополнительныеДанные=новый ХранилищеЗначения(новый Структура("ТаблицаДляГруппировкиДанных,ДвижениеЦелойПаллеты,СистемнаяЯчейка",ТаблицаДляГруппировкиДанных,
		ВыборкаИдентификаторУпаковкиПолучатель.ДвижениеЦелойПаллеты,СистемнаяЯчейка));
	КонецЦикла;
	НаборЗаписей.Записать();
...
Процедура ЗаполнитьСтрокиЗадачиТСД(Отказ, РежимПроведения,ИдЗадачи,ВыборкаИдентификаторУпаковкиПолучатель,ТаблицаДляГруппировкиДанных)
...
КонецПроцедуры
Процедура ПеревестиЗаказВСостояниеНабора()
	Если ТипЗнч(итОснование)=Тип("ДокументСсылка.ЗаказПокупателя") Тогда 
	...
	Пока ВыборкаПоЯчейке.Следующий() Цикл
		...
		Пока ВыборкаУпаковка.Следующий() Цикл   
			Если ВыборкаУпаковка.РезервированиеТовараВЯчейке Тогда 
				Сообщить("По идентификатору "+ВыборкаУпаковка.ИдентификаторУпаковки +" в ячейке хранения имеется резерв");
				Отказ=Истина;
				Продолжить;
			КонецЕсли;	
			...
			Пока ВыборкаДетальныйхЗаписей.Следующий() Цикл 
				Если ВыборкаДетальныйхЗаписей.Количество>ВыборкаДетальныйхЗаписей.КоличествоОстаток Тогда 
					Сообщить("по идентификатору "+ВыборкаДетальныйхЗаписей.ИдентификаторУпаковки+" не хватает номенклатуры "+Строка(ВыборкаДетальныйхЗаписей.Номенклатура) 
					+ " в количестве "+Строка(ВыборкаДетальныйхЗаписей.Количество-ВыборкаДетальныйхЗаписей.КоличествоОстаток));
					Отказ=Истина;
				КонецЕсли;
				НоваяЗапись=НаборЗаписей.Добавить();
...
Процедура ПроверкаНаСоответсвиеДанныхЯчеек(Отказ, РежимПроведения)
	...
	Пока ВыборкаЯчекиОтправитель.Следующий() Цикл
		Отказ=Истина;
		Сообщить("для идентификатора "+ВыборкаЯчекиОтправитель.ИдентификаторУпаковки+" превышено количество забираемого товара "+ВыборкаЯчекиОтправитель.Номенклатура+" в"+
		" количесте "+ВыборкаЯчекиОтправитель.НеХватает);
	КонецЦикла;
	
	Пока ВыборкаЯчейкиЭстокады.Следующий() Цикл
		Отказ=Истина;
		Сообщить("для ячейки эстокады "+ ВыборкаЯчейкиЭстокады.Ячейка+" превышено максимальное количество паллет "+ВыборкаЯчейкиЭстокады.Превышено);
	КонецЦикла;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт 
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя") или ТипЗнч(ДанныеЗаполнения)=Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
...
Процедура ОбработчикЗаполненияПоПриорететамСПИЗонам(МенеджерВременныхТаблиц,СтрокаОбхода,ДанныеЗаполнения,НастройкиWMS)
	...
	СтруктураПараметров=СтруктураПараметровЗаполненияПоЗонам(МенеджерВременныхТаблиц,СтрокаОбхода,ДанныеЗаполнения);
	Если _КоличествоНаПаллете=0 Тогда 
		итWMSОбщегоНазначенияКлиентСервер.СообщитьПользователю( "Для "+Строка(СтрокаОбхода.Номенклатура)+", ед.изм.мест "+СтрокаОбхода.ЕдиницаИзмеренияМест+" не указано количество в паллете." );
		Товары.Очистить();
		Возврат
	КонецЕсли;
...
Процедура ОбработчикЗаполненияСквознойПоСПиЗонам(МенеджерВременныхТаблиц,СтрокаОбхода,ДанныеЗаполнения,НастройкиСтратегии)
...
ОбластьЗаголовок.Параметры.ДатаОтгрузкиВремяПогрузки="Дата отгрузки "+Формат(итОснование.ДатаОтгрузки,"ДЛФ=D")+", Время погрузки "+Формат(итОснование.итВремяНачалаПогрузкиКакВремя,"ДЛФ=T");
...
ТабличныйДокумент.Вывести(ОбластьшапкаТаблицы);
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(ВтДанныеНаборки.КоличествоФакт / ВтДанныеНаборки.Номенклатура.ЕдиницаИзмеренияМест.Коэффициент + 0.499 КАК ЧИСЛО(15, 0)) КАК Мест,
	...
	Если  ВыборкаДетальныеЗаписи.Следующий() Тогда 
		для Каждого Строка из ВыборкаДетальныеЗаписи.Ссылка.Товары цикл
			...
			Если Строка.СостояниеЗадачи=Перечисления.итWMSСостоянияЗадачТСД.Выполнена Тогда 					
				МассивСтрокНаборки=Товары.НайтиСтроки(новый Структура("ИдентификаторУпаковки,ЯчейкаОтправитель",Строка.ИдентификаторУпаковки,Строка.ЯчейкаОтправитель));
...
Процедура АнализОшибокПМУ(МенеджерВременныхТаблиц,Филиал)
	...
	Пока ВыборкаДетальныхЗаписей.Следующий() цикл
		МассивСтрок=Товары.НайтиСтроки(новый Структура("ИдентификаторУпаковкиПолучатель",ВыборкаДетальныхЗаписей.ИдентификаторУпаковки));
...
Процедура СобратьПоОсколкам(ТаблицаОсколковНоменклатуры,ТаблицаДляАнализаИдентификатора,МассивСформированныхSSCC,ДополнительныеПараметры)
	...
	для Каждого стр из ТаблицаОсколковНоменклатуры цикл
		...
		Если МассивПозицийНоменклатурыНаПаллете.Найти(стр.Номенклатура)=Неопределено и (МассивПозицийНоменклатурыНаПаллете.Количество()+1)>КоличествоПозицийНоменклатурыНаПаллете тогда
			Продолжить;
		КонецЕсли;
		Если МассивСерийНаПаллете.Найти(стр.СерияНоменклатуры)=Неопределено и (МассивСерийНаПаллете.Количество()+1)>КоличествоПозицийСерииНаПаллете тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОпределитьЯчейкиЭстокадыДляВсехДокументовМаршрута(Заказ)
	...
	Если СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Выполнен или
		СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Завершен или
...
Функция СтатусДокументаПриФиксации()
	МассивСтрокОтмененныхЗадач=Товары.НайтиСтроки(новый Структура("СостояниеЗадачи,ФиксацияСтроки",Перечисления.итWMSСостоянияЗадачТСД.Отменена,Ложь));	
