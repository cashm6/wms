...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт  
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.итWMSПриемка") Тогда
		...
		Для Каждого ТекСтрокаТовары Из ДанныеЗаполнения.Товары Цикл
			...
			НоваяСтрока.ДатаРозлива = ТекСтрокаТовары.ДатаРозлива;
			НоваяСтрока.ИдентификаторСтроки = новый УникальныйИдентификатор;
			НоваяСтрока.ИдентификаторУпаковки = ТекСтрокаТовары.ИдентификаторУпаковки;
			НоваяСтрока.Качество = ТекСтрокаТовары.Качество;
			НоваяСтрока.Количество = ТекСтрокаТовары.КоличествоФакт;
			НоваяСтрока.Номенклатура = ТекСтрокаТовары.Номенклатура;
			НоваяСтрока.Характеристика = ТекСтрокаТовары.Характеристика;
			НоваяСтрока.ЯчейкаОтправитель = ТекСтрокаТовары.ЯчейкаПолучатель;
			НоваяСтрока.КарантинПаллеты=ТекСтрокаТовары.КарантинПаллеты;
...
Процедура ЗапросСвободныхЯчеекСогласноПаллетМестОпт(МенеджерЗапроса,СтруктураДанных,ВидСкладскойДеятельности=Неопределено)	
	...
	|	СУММА((итТоварыВЯчейкахОстатки.КоличествоОстаток + итТоварыВЯчейкахОстатки.КРазмещениюОстаток + ЕСТЬNULL(ДвиженияДокумента.КоличествоДвиженияДокумента, 0)) / ВЫБОР
...
Процедура ЗапросСвободныхЯчеекСогласноПаллетМестРозница(МенеджерЗапроса)	
	...
	|	(итТоварыВЯчейкахОстатки.КоличествоОстаток + итТоварыВЯчейкахОстатки.КРазмещениюОстаток + ДанныеДвиженияДокумента.ДвижениеДокумента) / ВЫБОР
	|		КОГДА ЕСТЬNULL(итТоварыВЯчейкахОстатки.Номенклатура.ЕдиницаИзмеренияМест.итКоличествоНаПаллете, 0) = 0
	|			ТОГДА 1
	|		ИНАЧЕ ЕСТЬNULL(итТоварыВЯчейкахОстатки.Номенклатура.ЕдиницаИзмеренияМест.итКоличествоНаПаллете, 0)
...
Процедура ИзменениеСтатусаОснования()
	
	Перем ОбъектОснование;
	
	Если не ЭтотОбъект.итОснование.Пустая() тогда
		ОбъектОснование=ЭтотОбъект.итОснование.ПолучитьОбъект();
		ОбъектОснование.ОпределитьСтатусВыполненногоДокумента();
...
Процедура ПередЗаписьюОтменаПроведения(Отказ, РежимЗаписи, РежимПроведения)
	Если СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Распределяется или  СтатусДокумента=Перечисления.итWMSСтатусыСкладскихДокументов.Зарезервирован  тогда
		ПередЗаписьюДействияОтменыПроведения();
	...
	КонецЕсли;	
КонецПроцедуры
Процедура ПередЗаписьюДействияОтменыПроведения()
	
	Перем БлокировкаДанных, НаборЗаписей, НаборЗаписей_2, стр, ЭлементБлокировки;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПроверкаПередПроведением(Отказ, РежимПроведения);
	КонецЕсли;
	Если Отказ  тогда
		Возврат
	КонецЕсли;
	...
	КонецЕсли;
	ПроверкаНаСоответствиеДанныхЯчеек(Отказ, РежимПроведения);
...
Процедура ПроверкаПередПроведением(Отказ, РежимПроведения)
	Если не итОснование.Пустая() тогда
		ПроверкаНаНесолькоДокументовРазмещенияУОснования(Отказ, РежимПроведения);
		Если Отказ тогда
			Возврат
		КонецЕсли;
		ПроверкаНаСоответсвиеДанныхПриемкиРазмещения(Отказ, РежимПроведения)
	КонецЕсли;
КонецПроцедуры

Процедура ПроверкаНаНесолькоДокументовРазмещенияУОснования(Отказ, РежимПроведения)
	...
	|ГДЕ
	|	итWMSРазмещение.итОснование = &итОснование
	|	И итWMSРазмещение.Ссылка <> &Ссылка
	...
	Если  ВыборкаДетальныеЗаписи.Следующий() тогда
			Сообщить("у документа приемки "+итОснование+" у жесть проведенное размещение");
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверкаНаСоответсвиеДанныхПриемкиРазмещения(Отказ, РежимПроведения)
	Если Склад<>итОснование.Склад Тогда 
		Сообщить("не соотв склада");
		Отказ=Истина;
	КонецЕсли;
	Если Организация<>итОснование.Организация Тогда 
		Сообщить("Не соотв организации");
		Отказ=Истина;
	КонецЕсли;	
	...
	|	итWMSПриемкаТовары.ЯчейкаПолучатель,
	|	0 КАК Количество,
	|	итWMSПриемкаТовары.СерияНоменклатуры
	|ПОМЕСТИТЬ ТаблицаОбъединенияДанных
	|ИЗ
	...
	|	итWMSРазмещениеТовары.ДатаРозлива,
	|	0,
	|	итWMSРазмещениеТовары.ЯчейкаОтправитель,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаОбъединенияДанных.Номенклатура КАК Номенклатура,
	|		ТаблицаОбъединенияДанных.Характеристика КАК Характеристика,
	|		ТаблицаОбъединенияДанных.Качество КАК Качество,
	|		ТаблицаОбъединенияДанных.ДатаРозлива КАК ДатаРозлива,
	|		СУММА(ТаблицаОбъединенияДанных.КоличествоФакт) КАК КоличествоФакт,
	|		ТаблицаОбъединенияДанных.ЯчейкаПолучатель КАК ЯчейкаПолучатель,
	|		СУММА(ТаблицаОбъединенияДанных.Количество) КАК Количество,
	|		ТаблицаОбъединенияДанных.СерияНоменклатуры КАК СерияНоменклатуры
	|	ИЗ
	|		ТаблицаОбъединенияДанных КАК ТаблицаОбъединенияДанных
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ТаблицаОбъединенияДанных.Номенклатура,
	|		ТаблицаОбъединенияДанных.Характеристика,
	|		ТаблицаОбъединенияДанных.Качество,
	|		ТаблицаОбъединенияДанных.ДатаРозлива,
	|		ТаблицаОбъединенияДанных.ЯчейкаПолучатель,
	|		ТаблицаОбъединенияДанных.СерияНоменклатуры) КАК ВложенныйЗапрос
	|ГДЕ
	|	ВложенныйЗапрос.Количество <> ВложенныйЗапрос.КоличествоФакт";
	
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("Основание",итОснование);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Отказ=Истина;
			Сообщить("есть расхождения между документов приемки и размещения по номелкатуре " +ВыборкаДетальныеЗаписи.Номенклатура +" с датой розлива "+ВыборкаДетальныеЗаписи.ДатаРозлива + " и кодом серии "+ВыборкаДетальныеЗаписи.СерияНоменклатуры.Код);
	КонецЦикла;
...
Процедура РаспределениеДокументаНаЗадачиТСД(Отказ, РежимПроведения)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	итWMSРазмещениеТовары.Ссылка,
	|	итWMSРазмещениеТовары.НомерСтроки,
	|	итWMSРазмещениеТовары.ИдентификаторСтроки,
	|	итWMSРазмещениеТовары.ИдентификаторУпаковки КАК ИдентификаторУпаковки,
	|	итWMSРазмещениеТовары.Номенклатура,
	|	итWMSРазмещениеТовары.Характеристика,
	|	итWMSРазмещениеТовары.Качество,
	|	итWMSРазмещениеТовары.ДатаРозлива,
	|	итWMSРазмещениеТовары.ЯчейкаПолучатель КАК ЯчейкаПолучатель,
	|	итWMSРазмещениеТовары.СостояниеЗадачи,
	...
	Пока ВыборкаИдентификаторУпаковки.Следующий() Цикл
		...
		НоваяЗапись.ТипЗадачи=Перечисления.итWMSТипыЗадачТСД.Размещение;
...
Процедура РезервированиеТоваровВЯчейкахЯчейкаОтправитель(Отказ,РежимПроведения,СнятиеСРезерва)
	...
	для Каждого стр из Товары цикл
		...
		НоваяЗапись.ВидДвижения=?(СнятиеСРезерва,ВидДвиженияНакопления.Расход,ВидДвиженияНакопления.Приход);
...
//    МенеджерВременныхТаблиц=новый МенеджерВременныхТаблиц;
//	КоличествоПаллет(МенеджерВременныхТаблиц);	
//	Запрос = Новый Запрос;
//	Запрос.МенеджерВременныхТаблиц=МенеджерВременныхТаблиц;
//	Запрос.Текст = "ВЫБРАТЬ
//	|	итТоварыВЯчейкахОстатки.Ячейка КАК Ячейка,
//	|	итТоварыВЯчейкахОстатки.КоличествоОстаток + итТоварыВЯчейкахОстатки.КРазмещениюОстаток КАК КоличествоОстаток,
//	|	итТоварыВЯчейкахОстатки.Номенклатура КАК Номенклатура,
//	|	итТоварыВЯчейкахОстатки.Номенклатура.ЕдиницаИзмеренияМест.итКоличествоНаПаллете КАК КоличествоНоменклатурыНаПаллете
//	|ПОМЕСТИТЬ ТаблицаАнализаДанных
//	|ИЗ
//	|	РегистрНакопления.итТоварыВЯчейках.Остатки(&Период, Ячейка В
//	|		(ВЫБРАТЬ
//	|			КолчествоПаллетКРазмещениюВЯчейку.Ячейка
//	|		ИЗ
//	|			КолчествоПаллетКРазмещениюВЯчейку КАК КолчествоПаллетКРазмещениюВЯчейку)) КАК итТоварыВЯчейкахОстатки
//	|;
//	|
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
//	|	ТаблицаАнализаДанных.Ячейка,
//	|	СУММА(ЕСТЬNULL(ТаблицаАнализаДанных.КоличествоОстаток, 0) / ВЫБОР
//	|		КОГДА ЕСТЬNULL(ТаблицаАнализаДанных.КоличествоНоменклатурыНаПаллете, 0) = 0
//	|			ТОГДА 1
//	|		ИНАЧЕ ТаблицаАнализаДанных.КоличествоНоменклатурыНаПаллете
//	|	КОНЕЦ) КАК КочествоПаллетВЯчейке
//	|ПОМЕСТИТЬ ПаллетыВЯчейках
//	|ИЗ
//	|	ТаблицаАнализаДанных КАК ТаблицаАнализаДанных
//	|СГРУППИРОВАТЬ ПО
//	|	ТаблицаАнализаДанных.Ячейка
//	|;
//	|
...
//	|ВЫБРАТЬ
//	|	КолчествоПаллетКРазмещениюВЯчейку.Ячейка,
//	|	КолчествоПаллетКРазмещениюВЯчейку.КоличествоПаллетКРазмещению,
//	|	КолчествоПаллетКРазмещениюВЯчейку.МаксимальноеКоличесвтоПаллетВЯчейке,
//	|	ПаллетыВЯчейках.КочествоПаллетВЯчейке,
//	|	ВЫБОР
//	|		КОГДА ЕСТЬNULL(КолчествоПаллетКРазмещениюВЯчейку.МаксимальноеКоличесвтоПаллетВЯчейке, 0) -
//	|			ЕСТЬNULL(ПаллетыВЯчейках.КочествоПаллетВЯчейке, 0) < КолчествоПаллетКРазмещениюВЯчейку.КоличествоПаллетКРазмещению
//	|			ТОГДА ИСТИНА
//	|		ИНАЧЕ ЛОЖЬ
//	|	КОНЕЦ КАК НеХватаетМеста,
//	|	ВЫБОР
//	|		КОГДА КолчествоПаллетКРазмещениюВЯчейку.МаксимальноеКоличесвтоПаллетВЯчейке = 0
//	|			ТОГДА ИСТИНА
//	|		ИНАЧЕ ЛОЖЬ
//	|	КОНЕЦ КАК НеУказаннаРазмерностьЯчейки
//	|ИЗ
//	|	КолчествоПаллетКРазмещениюВЯчейку КАК КолчествоПаллетКРазмещениюВЯчейку
//	|		ЛЕВОЕ СОЕДИНЕНИЕ ПаллетыВЯчейках КАК ПаллетыВЯчейках
//	|		ПО КолчествоПаллетКРазмещениюВЯчейку.Ячейка = ПаллетыВЯчейках.Ячейка";
//	
//	Запрос.УстановитьПараметр("Период", новый Граница(новый МоментВремени(ПолучитьДатуДвиженияДокумента().ДатаОстатков,Ссылка),ВидГраницы.Исключая));
//	Запрос.УстановитьПараметр("Ссылка",Ссылка);	
//	
//	РезультатЗапроса = Запрос.Выполнить();
//	
...
//	
//	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
//		Если  ВыборкаДетальныеЗаписи.НеХватаетМеста тогда
//			Если ОбработкаТСД Тогда 
//				Отказ=Истина;
//				СообщениеДляТСД=СообщениеДляТСД+"
//				|"+"ячейка "+ ВыборкаДетальныеЗаписи.Ячейка+" уже занята, пожалуйста выберите другую или воспользуйтесь механизмом размещения паллет еще раз";
//			иначе
//				Сообщить("ячейка "+ ВыборкаДетальныеЗаписи.Ячейка+" уже занята, пожалуйста выберите другую или воспользуйтесь механизмом размещения паллет еще раз");
//				Отказ=Истина;
//			КонецЕсли;
//		КонецЕсли;	
//		Если  ВыборкаДетальныеЗаписи.НеУказаннаРазмерностьЯчейки тогда
//			Если ОбработкаТСД Тогда 
//				Отказ=Истина;
//				СообщениеДляТСД=СообщениеДляТСД+"
//				|"+"для ячейки "+ ВыборкаДетальныеЗаписи.Ячейка+" не указанно количество паллет";
//			иначе
//				Сообщить("для ячейки "+ ВыборкаДетальныеЗаписи.Ячейка+" не указанно количество паллет");
//				Отказ=Истина;
//			КонецЕсли;
//		КонецЕсли;	
//		
...
Процедура ПроверкаНаСоответствиеДанныхЯчеек(Отказ, РежимПроведения)
	...
	Запрос.Текст =
		...
		|	итТоварыВЯчейкахОстатки.Ячейка КАК Ячейка,
		|	итТоварыВЯчейкахОстатки.КоличествоОстаток + итТоварыВЯчейкахОстатки.КРазмещениюОстаток КАК КоличествоОстаток,
		|	итТоварыВЯчейкахОстатки.Номенклатура.ЕдиницаИзмеренияМест.итКоличествоНаПаллете КАК КоличествоНаПаллете,
		|	итТоварыВЯчейкахОстатки.ИдентификаторУпаковки КАК ИдентификаторУпаковки
		|ПОМЕСТИТЬ ВтДанныеОстаток
		|ИЗ
		|	РегистрНакопления.итТоварыВЯчейках.Остатки(
		|			&Период,
		|			Ячейка В
		|				(ВЫБРАТЬ
		|					ВтЯчейкиРазмещения.ЯчейкаПолучатель
		|				ИЗ
		|					ВтЯчейкиРазмещения КАК ВтЯчейкиРазмещения)) КАК итТоварыВЯчейкахОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтДанныеОстаток.Ячейка КАК Ячейка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВтДанныеОстаток.ИдентификаторУпаковки) КАК КоличестоИдентификаторов
		|ПОМЕСТИТЬ КоличествоИдентификаторовВЯчейке
		|ИЗ
		|	ВтДанныеОстаток КАК ВтДанныеОстаток
		|
		|СГРУППИРОВАТЬ ПО
		|	ВтДанныеОстаток.Ячейка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтДанныеОстаток.Ячейка КАК Ячейка,
		|	СУММА(ВтДанныеОстаток.КоличествоОстаток / ВЫБОР
		|			КОГДА ЕСТЬNULL(ВтДанныеОстаток.КоличествоНаПаллете, 0) = 0
		|				ТОГДА 1
		|			ИНАЧЕ ЕСТЬNULL(ВтДанныеОстаток.КоличествоНаПаллете, 0)
		|		КОНЕЦ) КАК КоэффициентПаллетВЯчейке
		|ПОМЕСТИТЬ КоэфициентныйОстатокВПаллете
		|ИЗ
		|	ВтДанныеОстаток КАК ВтДанныеОстаток
		|
		|СГРУППИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтЯчейкиРазмещения.ЯчейкаПолучатель КАК ЯчейкаПолучатель,
		|	ЕСТЬNULL(КоличествоИдентификаторовВЯчейке.КоличестоИдентификаторов, 0) КАК КоличестоИдентификаторов,
		|	ЕСТЬNULL(КоэфициентныйОстатокВПаллете.КоэффициентПаллетВЯчейке, 0) КАК КоэффициентПаллетВЯчейке,
		|	ВтЯчейкиРазмещения.ЯчейкаПолучатель.КоличествоПалет КАК МаксимальноеКоличествоПаллет,
		|	ВтЯчейкиРазмещения.ЯчейкаПолучатель.ВидСкладскойДеятельности КАК ВидСкладскойДеятельности
		|ИЗ
		|	ВтЯчейкиРазмещения КАК ВтЯчейкиРазмещения
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоИдентификаторовВЯчейке КАК КоличествоИдентификаторовВЯчейке
		|		ПО ВтЯчейкиРазмещения.ЯчейкаПолучатель = КоличествоИдентификаторовВЯчейке.Ячейка
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоэфициентныйОстатокВПаллете КАК КоэфициентныйОстатокВПаллете
		|		ПО ВтЯчейкиРазмещения.ЯчейкаПолучатель = КоэфициентныйОстатокВПаллете.Ячейка
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ВтЯчейкиРазмещения.ЯчейкаПолучатель.ВидСкладскойДеятельности = ЗНАЧЕНИЕ(Перечисление.ит_WMS_ВидыСкладскойДеятельности.Оптовая)
		|				ТОГДА ВтЯчейкиРазмещения.ЯчейкаПолучатель.КоличествоПалет < ЕСТЬNULL(КоличествоИдентификаторовВЯчейке.КоличестоИдентификаторов, 0)
		|			ИНАЧЕ ВтЯчейкиРазмещения.ЯчейкаПолучатель.КоличествоПалет < ЕСТЬNULL(КоэфициентныйОстатокВПаллете.КоэффициентПаллетВЯчейке, 0)
		|		КОНЕЦ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если не РольДоступна("ИгнорироватьОшибки") Тогда 
		Отказ=Истина;
		КонецЕсли;
		Если ВыборкаДетальныеЗаписи.ВидСкладскойДеятельности=Перечисления.ит_WMS_ВидыСкладскойДеятельности.Розничная Тогда
			Сообщить( "В ячейке "+Строка(ВыборкаДетальныеЗаписи.ЯчейкаПолучатель)+" превышено максимальное количество паллет на "+Строка(ВыборкаДетальныеЗаписи.КоэффициентПаллетВЯчейке-ВыборкаДетальныеЗаписи.МаксимальноеКоличествоПаллет) );
		Иначе
			Сообщить( "В ячейке "+Строка(ВыборкаДетальныеЗаписи.ЯчейкаПолучатель)+" превышено максимальное количество паллет на "+Строка(ВыборкаДетальныеЗаписи.КоличестоИдентификаторов-ВыборкаДетальныеЗаписи.МаксимальноеКоличествоПаллет) );
		КонецЕсли;
	КонецЦикла;
...
//Процедура КоличествоПаллет(МенеджерВременныхТаблиц)
//	Попытка
//		ДанныеХранилища=ПараметрыСтратегии.Получить();
//	Исключение
//		ДанныеХранилища=Неопределено;
//	КонецПопытки;
//	ВидСкладскойДеятельности=Неопределено;
//	Если ТипЗнч(ДанныеХранилища)=Тип("Массив") Тогда  
//		ВидСкладскойДеятельности=НайтиЗначенияВСтруктурированномМассиве(ДанныеХранилища,"ВидСкладскойДеятельности");
//	КонецЕсли;
//	Если ВидСкладскойДеятельности=Перечисления.ит_WMS_ВидыСкладскойДеятельности.Розничная Тогда 
//		КоличествоПаллетРозница(МенеджерВременныхТаблиц);
//	Иначе 
//		КоличествоПаллетОпт(МенеджерВременныхТаблиц);
//	КонецЕсли;
//КонецПроцедуры
...
Процедура ПечатьМакетВсеНаОдномЛисте(ТабличныйДокумент,СсылкаНаДокумент) Экспорт 
	...
	|	итWMSРазмещениеТовары.Номенклатура КАК Наименование,
	|	итWMSРазмещениеТовары.ДатаРозлива КАК Дата,
	|	СУММА(итWMSРазмещениеТовары.Количество) КАК Количество,
	|	итWMSРазмещениеТовары.ЯчейкаПолучатель КАК Ячейка
	|ИЗ
	|	Документ.итWMSРазмещение.Товары КАК итWMSРазмещениеТовары
	|ГДЕ
	|	итWMSРазмещениеТовары.Ссылка = &Ссылка
