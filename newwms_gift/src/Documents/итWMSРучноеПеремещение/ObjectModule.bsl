...
Процедура ПроверкаНаСоответсвиеДанныхЯчеек(Отказ, РежимПроведения)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	итWMSРучноеПеремещениеТовары.ИдентификаторУпаковки,
		|	итWMSРучноеПеремещениеТовары.Номенклатура,
		|	итWMSРучноеПеремещениеТовары.СерияНоменклатуры,
		|	итWMSРучноеПеремещениеТовары.ЯчейкаОтправитель,
		|	итWMSРучноеПеремещениеТовары.ЯчейкаПолучатель,
		|	итWMSРучноеПеремещениеТовары.Количество,
		|	итWMSРучноеПеремещениеТовары.Ссылка.Организация,
		|	итWMSРучноеПеремещениеТовары.Ссылка.СкладОтправитель,
		|	итWMSРучноеПеремещениеТовары.Ссылка.СкладПолучатель,
		...
		|ГДЕ
		|	итWMSРучноеПеремещениеТовары.Ссылка = &Ссылка
		|;
		|
		|
		|
...
Процедура ПоискЯчеекРазмещения(СтруктураДанных)Экспорт
	...
	НастройкиWMS=итWMSПривилегированныйМодуль.ПолучитьНастройкиИзХранилищаПоСвойствам("ЗонаКарантина,ЯчейкиИсключенияАвтоматическогоРазмещения");
	...
	КонецЕсли;
	//////////////////////////////////
	ЗонаКарантина=НастройкиWMS.ЗонаКарантина;
...
Процедура ЗаполнитьДанныеСвободнымиЯчейкамиКарантина(СкладскоеПомещение,МенеджерЗапроса)
	...
	|		ЕСТЬNULL(итТоварыВЯчейкахРезервОстатки.КоличествоОстаток, 0) / ВЫБОР
	|			КОГДА ЕСТЬNULL(ВТНомеклатураНаПаллете.итКоличествоНаПаллете, 1) = 0
	|				ТОГДА 1
	|			ИНАЧЕ ЕСТЬNULL(ВТНомеклатураНаПаллете.итКоличествоНаПаллете, 1)
	|		КОНЕЦ
	|	ИЗ
	|		РегистрНакопления.итТоварыВЯчейкахРезерв.Остатки КАК итТоварыВЯчейкахРезервОстатки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНомеклатураНаПаллете КАК ВТНомеклатураНаПаллете
	|			ПО итТоварыВЯчейкахРезервОстатки.Номенклатура = ВТНомеклатураНаПаллете.Номенклатура) КАК ВложенныйЗапрос
	|ГДЕ
	...
	|ВЫБРАТЬ
	|	итСкладскиеЯчейки.Ссылка КАК Ячейка,
	|	ЕСТЬNULL(итСкладскиеЯчейки.КоличествоПалет, 0) - ЕСТЬNULL(ПаллетыВЯчейках.ПаллетВЯчейке, 0) -
	|		ЕСТЬNULL(ВтРассчетЗанятыхПаллетоМест.ЗанятоеКоличествоПаллетоМест, 0) КАК КоличествоПаллет,
	|	итСкладскиеЯчейки.СкладскоеПомещение,
	|	итСкладскиеЯчейки.Секция,
	|	итСкладскиеЯчейки.Стеллаж,
	|	итСкладскиеЯчейки.Линия,
	...
	|ГДЕ
	|	ЕСТЬNULL(итСкладскиеЯчейки.КоличествоПалет, 0) - ЕСТЬNULL(ПаллетыВЯчейках.ПаллетВЯчейке, 0) -
	|		ЕСТЬNULL(ВтРассчетЗанятыхПаллетоМест.ЗанятоеКоличествоПаллетоМест, 0) >= 1
