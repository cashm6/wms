...
ОповеститьОВыборе(итWMSОбработчикSSCC.ПолучитьНовыйSSCC(Филиал,,ФизЛицоОтветственного(Ответственный)));
КонецПроцедуры
&НаСервереБезКонтекста
Функция ФизЛицоОтветственного(Ответственный)
	Возврат  Ответственный.ФизическоеЛицо;
	КонецФункции
&НаСервере
